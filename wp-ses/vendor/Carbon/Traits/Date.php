...
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\BadComparisonUnitException;
...
use DeliciousBrains\WP_Offload_SES\Symfony\Component\Clock\NativeClock;
use Throwable;
/**
...
trait Date
{
                ...
                if (\preg_match('/^([a-z]{2,})Of([A-Z][a-z]+)$/', $name, $match)) {
                    ...
                    try {
                        ...
                        $currentValue = (\in_array($firstUnit, [
                            ...
                            'month',
                            'quarter',
                        ], \true) ? 1 : 0) + (int) \floor($start->diffInUnit($firstUnit, $result));
    ...
    public function setUnit(string $unit, Month|int|float|null $value = null, OverflowMode|bool|null $overflow = null, ?int $anchorDay = null) : static
    {
        ...
        if (\in_array($unit, $dateUnits)) {
            if ($unit !== 'day') {
                $overflow ??= $this->shouldUnitOverflow($unit) ? OverflowMode::Overflow : OverflowMode::NoOverflow;
                ...
                if ($anchorDay !== null) {
                    $date = $this->setDate(...\array_map(fn($name) => (int) match ($name) {
                        $unit => $value,
                        'day' => 1,
                        default => $this->{$name},
                    }, $dateUnits));
    ...
    private function callModifierMethod(string $unit, array $parameters) : ?static
    {
        ...
        if (static::isModifiableUnit($unit)) {
            ...
            if ($anchorDay !== null && $overflow) {
                throw new InvalidArgumentException('$anchorDay is not compatible with overflow');
            }
            return $this->{"{$action}Unit"}($unit, $this->getMagicParameter($parameters, 0, 'value', 1), $anchorDay === null ? $overflow : OverflowMode::AnchorDay, $anchorDay);
        }
        return null;
    }
