...
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\InvalidTypeException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\NotLocaleAwareException;
use DeliciousBrains\WP_Offload_SES\Carbon\Language;
use DeliciousBrains\WP_Offload_SES\Carbon\Translator;
use DeliciousBrains\WP_Offload_SES\Carbon\TranslatorStrongTypeInterface;
use Closure;
...
trait Localization
{
    ...
    public static function setFallbackLocale(string $locale) : void
    {
        ...
        if (\method_exists($translator, 'setFallbackLocales')) {
            ...
            if (\method_exists($translator, 'getFallbackLocales') && $fallbackLocales === $translator->getFallbackLocales()) {
                return;
            }
            $translator->setFallbackLocales($fallbackLocales);
