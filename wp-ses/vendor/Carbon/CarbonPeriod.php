 ...
 */
namespace DeliciousBrains\WP_Offload_SES\Carbon;

use DeliciousBrains\WP_Offload_SES\Carbon\Constants\UnitValue;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\EndLessPeriodException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\InvalidCastException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\InvalidIntervalException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\InvalidPeriodDateException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\InvalidPeriodParameterException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\NotACarbonClassException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\NotAPeriodException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\UnknownGetterException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\UnknownMethodException;
use DeliciousBrains\WP_Offload_SES\Carbon\Exceptions\UnreachableException;
use DeliciousBrains\WP_Offload_SES\Carbon\Traits\DeprecatedPeriodProperties;
use DeliciousBrains\WP_Offload_SES\Carbon\Traits\IntervalRounding;
use DeliciousBrains\WP_Offload_SES\Carbon\Traits\LocalFactory;
use DeliciousBrains\WP_Offload_SES\Carbon\Traits\Mixin;
use DeliciousBrains\WP_Offload_SES\Carbon\Traits\Options;
...
class CarbonPeriod extends DatePeriodBase implements Countable, JsonSerializable, UnitValue
{
    ...
    public static function monthly(DateTimeInterface|string|int|null $start = null, DateTimeInterface|string|int|null $end = null, ?int $recurrences = null, ?int $anchorDay = null, OverflowMode $mode = OverflowMode::AnchorDay, ?int $options = null) : static
    {
        [$start, $end] = self::getStartAndEndForCyclePeriod($start, $end, $recurrences, $anchorDay, $mode);
        return (new static($start, self::getMonthInterval($start, $anchorDay, $mode), $end ?? $recurrences))->setOptions($options ?? self::IMMUTABLE);
    }
    public static function quarterly(DateTimeInterface|string|int|null $start = null, DateTimeInterface|string|int|null $end = null, ?int $recurrences = null, ?int $anchorDay = null, OverflowMode $mode = OverflowMode::AnchorDay, ?int $options = null) : static
    {
        [$start, $end] = self::getStartAndEndForCyclePeriod($start, $end, $recurrences, $anchorDay, $mode);
        $interval = self::getMonthInterval($start, $anchorDay, $mode);
        return (new static($start, static function (CarbonInterface $date, bool $negated) use($interval) : CarbonInterface {
            for ($i = 0; $i < CarbonInterface::MONTHS_PER_QUARTER; $i++) {
                $date = $negated ? $date->sub($interval) : $date->add($interval);
            }
            return $date;
        }, $end ?? $recurrences))->setOptions($options ?? self::IMMUTABLE);
    }
    public static function yearly(DateTimeInterface|string|int|null $start = null, DateTimeInterface|string|int|null $end = null, ?int $recurrences = null, ?int $anchorDay = null, OverflowMode $mode = OverflowMode::AnchorDay, ?int $options = null) : static
    {
        [$start, $end] = self::getStartAndEndForCyclePeriod($start, $end, $recurrences, $anchorDay, $mode);
        return (new static($start, match ($mode) {
            OverflowMode::AnchorDay => CarbonInterval::yearWithAnchorDay($anchorDay ?? $start->day),
    ...
    private static function getStartAndEndForCyclePeriod(DateTimeInterface|string|int|null $start = null, DateTimeInterface|string|int|null $end = null, ?int $recurrences = null, ?int $anchorDay = null, OverflowMode $mode = OverflowMode::AnchorDay) : array
    {
        if ($anchorDay !== null && $mode !== OverflowMode::AnchorDay) {
            throw new InvalidArgumentException('$anchorDay parameter must not be set for $mode OverflowMode::' . $mode->name);
        }
        if ($end !== null && $recurrences !== null) {
            throw new InvalidArgumentException('You must specify $end or $recurrences but not both');
        }
        if (\is_int($start)) {
    ...
    private static function getMonthInterval(DateTimeInterface|string|int|null $start = null, ?int $anchorDay = null, OverflowMode $mode = OverflowMode::AnchorDay) : CarbonInterval
    {
        return match ($mode) {
            OverflowMode::AnchorDay => CarbonInterval::monthWithAnchorDay($anchorDay ?? $start->day),
    ...
    public function __construct(...$arguments)
    {
        ...
        parent::__construct($this->startDate, $this->dateInterval, $this->endDate ?? \max(1, \min(2147483639, $this->recurrences ?? 1)), $this->options);
    ...
    public function setEndDate(mixed $date, ?bool $inclusive = null) : static
    {
        ...
        $self->endDate = $date;
        if ($self->startDate !== null && $self->dateInterval !== null && !$self->dateInterval->invert && $self->startDate > $self->endDate) {
    ...
    protected function syncNativePeriod() : void
    {
        ...
        $interval = $this->dateInterval ?? \DeliciousBrains\WP_Offload_SES\Carbon\CarbonInterval::day();
        ...
        parent::__construct($this->startDate, $interval, $this->endDate ?? \max(1, \min(2147483639, $this->recurrences ?? 1)), $this->options ?? 0);
    }
    /**
    ...
    protected function incrementCurrentDateUntilValid() : void
    {
        $attempts = 0;
        do {
            $this->carbonCurrent = $this->carbonCurrent->add($this->dateInterval, $this->dateInterval->getStep() && $this->dateInterval->invert ? -1 : 1);
            $this->validationResult = null;
    ...
    private function initializeSerialization(array $values) : void
    {
        ...
        $excludeStart = !($values['include_start_date'] ?? \true);
        $includeEnd = $values['include_end_date'] ?? \true;
        parent::__construct($serializationBase['start'], $serializationBase['interval'], $serializationBase['end'] ?? $serializationBase['recurrences'], ($excludeStart ? self::EXCLUDE_START_DATE : 0) | ($includeEnd && \defined('DatePeriod::INCLUDE_END_DATE') ? self::INCLUDE_END_DATE : 0));
        // @codeCoverageIgnoreEnd
    }
    private static function addFilterOrHandleChangedParameters(self $period, array|callable|string $filter) : self
    {
